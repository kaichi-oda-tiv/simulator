...
public class FindComponent
{
    ...
    {
        Debug.Log($"{typeof(CallHello).AssemblyQualifiedName} / {typeof(CallHello).Assembly.FullName}");
    ...
    {
        Debug.Log($"{typeof(CallHello).AssemblyQualifiedName} / {typeof(CallHello).Assembly.FullName}");
        new FindComponent().SearchComponentString(Selection.activeGameObject, "PackageName.CallHello");
    }

    [MenuItem("Assets/FindComponentStringB", false, 20), MenuItem("GameObject/FindComponentStringB", false, 20)]
    static void FindComponentStringB()
    {
        Debug.Log($"{typeof(CallHello).AssemblyQualifiedName} / {typeof(CallHello).Assembly.FullName}");
        new FindComponent().SearchComponentStringB(Selection.activeGameObject, "CallHello");
    }
    ...
    void SearchComponentStringB(GameObject root, string classname)
    {
        Debug.Log($"{classname} / " + (System.Type.GetType(classname) == null ? "null!" : "yes"));

        var cn = System.AppDomain.CurrentDomain.GetAssemblies().SelectMany(a => a.GetTypes()).Where(x => x.Name.EndsWith(classname)).FirstOrDefault();
        Debug.Log($"{classname} / " + (cn == null ? "null!" : cn.Name));

        var list = root.GetComponentsInChildren(cn, true);
        Log(list.Select(x => x.ToString()));
    }
