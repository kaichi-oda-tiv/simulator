    ...
    public partial class Bridge : IBridge
    {
        ...
        public void AddReader<T>(string topic, Action<T> callback) where T : class
        {
            ...
            else if (type == typeof(VehicleControlData))
            {
                ...
                // type = typeof(VehicleControlCommand);
                // reader = new Reader<VehicleControlCommand, VehicleControlData>(Conversions.ConvertTo, QueuedActions);

                type = typeof(RawControlCommand);
                reader = new Reader<RawControlCommand, VehicleControlData>(Conversions.ConvertTo, QueuedActions);
            }
            else if (type == typeof(VehicleStateData))
            {
                type = typeof(VehicleStateCommand);
                reader = new Reader<VehicleStateCommand, VehicleStateData>(Conversions.ConvertTo, QueuedActions);
            }
            else
            {
        ...
        public IWriter<T> AddWriter<T>(string topic) where T : class
        {
            ...
            else if (type == typeof(CanBusData))
            {
                ...
                writer = new Writer<CanBusData, VehicleStateReport>(this, topic, Conversions.ConvertFrom) as IWriter<T>;
            }
