    ...
    public class VideoRecordingSensor : SensorBase
    {
        ...
        public bool StartRecording(string outfile = null)
        {
            if (IsInit && !IsRecording)
            {
                ...
                int currentWidth = Camera.targetTexture.width;
                int currentHeight = Camera.targetTexture.height;

                if (Recorder.Start(Camera.targetTexture, currentWidth, currentHeight, Framerate, Bitrate, MaxBitrate, Quality, 0, destination))
                {
                    StartCoroutine(Recorder.OnRecord(Camera.targetTexture, currentWidth, currentHeight, Framerate, OnRecordingFailed));
                }
                else
                {
                    return false;
